...
export function middleware(request: NextRequest) {
  ...
  // Pre-signup routes: reachable with no session, no onboarding gate.
  ...
  // Not logged in → Get Started page (reviews Terms/Privacy before sign-in)
  ...
  // Logged in but on the sign-in or welcome page → dashboard
  if (hasSession && (pathname === "/auth/sign-in" || isWelcomeRoute)) {
    return NextResponse.redirect(new URL("/", request.url));
  }

  // Logged in but hasn't completed onboarding → /onboard
  // Skip check for public routes, onboard itself, and API routes
  if (hasSession && !isPublicRoute && !isOnboardRoute && !isApiRoute) {
